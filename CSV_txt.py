...
#
# # CSV文件路径
# csv_file_path = 'F:/kinetics/kinetics-dataset/k400/val.csv'
# # TXT文件路径
# txt_file_path = 'F:/kinetics/kinetics-dataset/k400/kinetics-400_annotations.txt'
#
# # 存储所有注释信息的列表
# annotations = []
#
...
#         video_id, label = row[1], row[0]  # 假设第一列是视频ID，第二列是标签
#         # 根据视频ID构造视频文件路径，这里需要根据实际文件结构调整
#         video_file_path = os.path.join('F:/kinetics/kinetics-dataset/k400/val', video_id + '.mp4')
#         # 将视频路径和标签添加到注释列表
#         annotations.append((video_file_path, label))
#
# # 写入TXT文件
# with open(txt_file_path, 'w') as txt_file:
#     for video_path, label in annotations:
#         txt_file.write(f"{video_path} {label}\n")
...
# CSV文件路径
csv_file_path = 'F:/kinetics/kinetics-dataset/k400/test.csv'
# TXT文件路径
txt_file_path = 'F:/kinetics/kinetics-dataset/k400/annotations/test.txt'
...
with open(csv_file_path, 'r') as csv_file, open(txt_file_path, 'w') as txt_file:
    ...
    next(csv_reader)  # 跳过标题行
    for row in csv_reader:
        video_id, category_name = row[1], row[0]  # 假设第一列是视频ID，第二列是类别名称
        # 将类别名称转换为数字标签
        label = label_map.get(category_name, -1)  # 如果类别名称不在映射中，返回-1或其他占位符
        # 假设视频文件名可以从视频ID构造出来
        video_filename = f"{video_id}.mp4"
        # 将视频文件名和对应的数字标签写入TXT文件
        txt_file.write(f"{video_filename} {label}\n")