...
def generate_annotation_file(video_folder, output_file, class_map):
    """
    根据视频文件夹，生成 Kinetics Tiny 风格的注释文件，其中类别名称使用数字编号。

    Args:
      video_folder: 包含视频文件的文件夹路径。
      output_file: 输出注释文件的路径。
      class_map: 类别名称到数字编号的映射字典。
    """
    # 获取视频文件夹的绝对路径
    ...
    for folder in os.listdir(video_folder):
        ...
        if os.path.isdir(folder_path):
            # 获取类别名称对应的数字编号
            label = class_map.get(folder, 0)
            # 遍历文件夹中的所有文件
            for file in os.listdir(folder_path):
                # 检查是否为 MP4 文件
                if file.endswith(".mp4"):
                    # 获取视频文件的绝对路径
                    video_file_abs = os.path.join(folder_path, file)
                    # 获取视频文件的相对路径
                    video_file_rel = os.path.relpath(video_file_abs, output_file_dir).replace("\\", "/")
                    video_files.append(f"{video_file_rel} {label}")
...
# 示例用法
video_folder = 'F:/kinetics/kinetics-dataset/k400/data/r'  # 替换为你的视频文件夹路径
output_file = 'F:/kinetics/kinetics-dataset/k400/data/train.txt'  # 输出注释文件的路径
...
generate_annotation_file(video_folder, output_file, class_map)
